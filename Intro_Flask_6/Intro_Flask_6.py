...
# Affichage de la liste des noms de la bdd
@app.route('/membres_groupe')
def membres():
    exo_db = mariadb.connect(host='localhost', user='root', password='root')
    cursor = exo_db.cursor()

    cursor.execute("USE data_user")
    #Selection des noms dans la bdd
    cursor.execute("SELECT nom FROM user")
    liste = cursor.fetchall() #fetchall récupère tous les noms s'ils existent, sinon renvoie None
    cursor.close()
    exo_db.close()
    
    return render_template("membres.html", liste = liste)
